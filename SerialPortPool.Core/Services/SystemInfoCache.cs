...
/// Phase 3: High-performance EEPROM data caching
/// </summary>
public class SystemInfoCache : IDisposable
{
    ...
    /// <summary>
    /// Internal cache entry with expiration and refresh tracking
    /// </summary>
    private class CacheEntry
    {
        ...
        public bool IsStale => IsExpired && !IsRefreshing;
    }
        ...
        TimeSpan? defaultTtl = null)
    {
        ...
        _defaultTtl = defaultTtl ?? TimeSpan.FromMinutes(5); // 5 min default as per plan
        ...
        try
        {
            ...
            if (!forceRefresh && _cache.TryGetValue(portName, out var cacheEntry))
            {
                ...
                else if (!cacheEntry.IsRefreshing)
                {
                    ...
                    cacheEntry.LastAccessed = DateTime.Now;
                    cacheEntry.AccessCount++;
                    Interlocked.Increment(ref _cacheHits); // Still count as hit (stale but available)
    ...
    /// </summary>
    public void InvalidateCache(string portName)
    {
        if (string.IsNullOrWhiteSpace(portName))
            return;
    ...
    private async Task<SystemInfo?> ReadSystemInfoFromDeviceAsync(string portName)
    {
        try
        {
            // Phase 3: Use existing EEPROM extension from ÉTAPE 3
            ...
            // Convert FTDI info to SystemInfo (leveraging ÉTAPE 3 model)
            var systemInfo = SystemInfo.FromFtdiDevice(ftdiInfo);
            
            // Add additional EEPROM data
