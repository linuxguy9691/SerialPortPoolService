...
public class EnhancedSerialPortDiscoveryService : ISerialPortDiscovery
{
    ...
    // Ajouter ces méthodes à la fin de la classe EnhancedSerialPortDiscoveryService
// dans le fichier SerialPortPool.Core/Services/EnhancedSerialPortDiscoveryService.cs
...
#region Private Helper Methods (Original Sprint 2 - PRESERVED)
    ...
    private async Task TestPortAccessibilityAsync(SerialPortInfo portInfo)
    {
        try
        {
            await Task.Run(() =>
            {
                using var serialPort = new System.IO.Ports.SerialPort(portInfo.PortName);
