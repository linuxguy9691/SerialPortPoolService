...
{
    /// <summary>
    /// Gestionnaire de protocole RS232 standard
    /// </summary>
    public class RS232ProtocolHandler : IProtocolHandler
    {
        ...
        #region IProtocolHandler Implementation
        ...
        public async Task<ProtocolSession> OpenSessionAsync(ProtocolConfiguration config, CancellationToken cancellationToken)
        {
            ...
            {
                throw new InvalidOperationException("Une session est déjà active. Fermez la session actuelle avant d'en ouvrir une nouvelle.");
            }

            try
            {
                _logger.LogInformation("Ouverture de session RS232 sur le port {PortName}", config.PortName);
                ...
                _logger.LogInformation("Session RS232 ouverte avec succès. SessionId: {SessionId}", _currentSession.SessionId);
                return _currentSession;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Erreur lors de l'ouverture de session RS232 sur {PortName}", config.PortName);
                throw;
            }
        }
        ...
        public async Task CloseSessionAsync(ProtocolSession session, CancellationToken cancellationToken)
        {
            ...
            try
            {
                _logger.LogInformation("Fermeture de session RS232. SessionId: {SessionId}", session.SessionId);
                ...
                if (session is CommunicationSession commSession)
                {
                    commSession.Status = "Disconnected";
                }

                _logger.LogInformation("Session RS232 fermée avec succès");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Erreur lors de la fermeture de session RS232");
                throw;
            }
        }
        ...
        public async Task<ProtocolResponse> SendCommandAsync(ProtocolRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                return ProtocolResponse.Error("Aucune session active");
            }
            ...
            try
            {
                _logger.LogDebug("Envoi de commande RS232: {Command}", request.Command);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Erreur lors de l'envoi de commande RS232");
                return ProtocolResponse.Error($"Erreur RS232: {ex.Message}");
            }
        }
        ...
        public async Task<IEnumerable<ProtocolResponse>> SendCommandSequenceAsync(IEnumerable<ProtocolRequest> requests, CancellationToken cancellationToken)
        {
            ...
            foreach (var request in requests)
            {
                ...
                if (!response.Success)
                {
                    _logger.LogWarning("Arrêt de la séquence de commandes RS232 en raison d'un échec");
                    break;
                }
            }
        ...
        public async Task<bool> TestConnectionAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                // Test simple: envoyer un caractère et vérifier la réponse
        ...
        public void Dispose()
        {
            ...
            {
                _logger.LogError(ex, "Erreur lors de la disposition du RS232ProtocolHandler");
            }
